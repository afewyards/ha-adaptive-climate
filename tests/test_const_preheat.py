...
def test_heating_type_preheat_config_has_required_keys():
    ...
    for heating_type, config in HEATING_TYPE_PREHEAT_CONFIG.items():
        assert set(config.keys()) == required_keys, f"{heating_type} missing required keys"
...
def test_fallback_rate_values_are_positive():
    ...
    for heating_type, config in HEATING_TYPE_PREHEAT_CONFIG.items():
        ...
        assert isinstance(fallback_rate, (int, float)), f"{heating_type} fallback_rate is not numeric"
        assert fallback_rate > 0, f"{heating_type} fallback_rate is not positive"
...
def test_max_hours_values_are_positive():
    ...
    for heating_type, config in HEATING_TYPE_PREHEAT_CONFIG.items():
        ...
        assert isinstance(max_hours, (int, float)), f"{heating_type} max_hours is not numeric"
        assert max_hours > 0, f"{heating_type} max_hours is not positive"
...
def test_cold_soak_margin_values_are_positive():
    ...
    for heating_type, config in HEATING_TYPE_PREHEAT_CONFIG.items():
        ...
        assert isinstance(cold_soak_margin, (int, float)), f"{heating_type} cold_soak_margin is not numeric"
        assert cold_soak_margin > 0, f"{heating_type} cold_soak_margin is not positive"
