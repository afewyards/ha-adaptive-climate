...
class TestChronicApproachHistoricScan:
    ...
    def test_historic_scan_disabled_by_default(self):
        ...
        if "chronic_approach_detector" in state:
            del state["chronic_approach_detector"]
        ...
        assert learner2._chronic_approach_detector._consecutive_failures == 0
        assert not learner2._chronic_approach_detector.should_adjust_ki()
    ...
    def test_historic_scan_enabled(self):
        ...
        # Add 3 consecutive chronic approach failures (radiator threshold)
        for _ in range(3):
            cycle = CycleMetrics(
                rise_time=None,
        ...
        assert learner2._chronic_approach_detector._consecutive_failures == 3
        assert learner2._chronic_approach_detector.should_adjust_ki()
    ...
    def test_historic_scan_respects_cooldown(self):
        ...
        detector = learner._chronic_approach_detector
        ...
        learner2.restore_from_dict(state)
        learner2._chronic_approach_detector.last_adjustment_time = detector.last_adjustment_time
        learner2._chronic_approach_detector.cumulative_ki_multiplier = detector.cumulative_ki_multiplier

        # Re-add cycles to detector after restoration
        for cycle in learner2._heating_cycle_history:
            learner2._chronic_approach_detector.add_cycle(cycle)

        # Should not recommend adjustment due to cooldown
        assert learner2._chronic_approach_detector.is_in_cooldown()
        assert not learner2._chronic_approach_detector.should_adjust_ki()
    ...
    def test_historic_scan_respects_cap(self):
        ...
        learner._chronic_approach_detector.cumulative_ki_multiplier = 1.95  # Near 2.0 cap
        ...
        learner2._chronic_approach_detector.cumulative_ki_multiplier = 1.95
        learner2.restore_from_dict(state)
    ...
    def test_historic_scan_multiple_heating_types(self):
        ...
        # Floor hydronic: 4 cycles, 0.4°C threshold, 60 min duration
        ...
        if "chronic_approach_detector" in state_floor:
            del state_floor["chronic_approach_detector"]
        ...
        assert learner_floor2._chronic_approach_detector._consecutive_failures == 4
        assert learner_floor2._chronic_approach_detector.should_adjust_ki()

        # Forced air: 2 cycles, 0.25°C threshold, 10 min duration
        ...
        if "chronic_approach_detector" in state_forced:
            del state_forced["chronic_approach_detector"]
        ...
        assert learner_forced2._chronic_approach_detector._consecutive_failures == 2
        assert learner_forced2._chronic_approach_detector.should_adjust_ki()
